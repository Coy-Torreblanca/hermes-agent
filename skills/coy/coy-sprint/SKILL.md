...
3. Propose POINTS, VALUE, SPRINT, destination file (tasks.org vs personal.org)
   - **🚨 Default SPRINT to backlog** unless the item is urgent/time-critical or Coy explicitly requested the current sprint. "Important" `[#A]` does not mean "sprint now." See gbrain `concepts/org-triage-backlog-default`.
...
- **🚨 ICEBOX sibling routing**: When an item conceptually belongs under an active EPIC (e.g., Personal AI v1 in Sprint 4) but is non-urgent housekeeping/improvement, do NOT place it there — route to the ICEBOX sibling EPIC instead (e.g., Personal AI v2 [ICEBOX]). Check `org_query.py --epics` for `[ICEBOX]`-suffixed EPICS with overlapping tags. Only use the active EPIC for urgent/time-critical items. Discovered May 18, 2026: 4 infrastructure items proposed for Sprint 4; Coy redirected them to Personal AI v2 [ICEBOX].
